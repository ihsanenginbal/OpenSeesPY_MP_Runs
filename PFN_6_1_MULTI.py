...
def eq_analysis(fac,filename,pid,record_no,dt_input):
    ...
    fc=-16800*fac[1]	    # Concrete Cylinder Characteristic Strength, kPa	(Reported by the field teams, based on the Schmidt's hammer - no core sampling was allowed in the severely damaged buildings)
    ...
    Heights=np.array([2.8/fac[5]*fac[4], 2.8, 2.7, 2.7, 2.7, 2.7])*fac[5]
    ...
    N0=np.array([[-124.4, -120.8, -48.00, -16.80],
                    [-120.4, -116.5, -45.80, -14.90],
                    [-114.4, -108.4, -43.60, -10.80],
                    [-109.8, -103.9, -43.00, -8.30],
                    [-109.0, -103.1, -41.40, -8.30],
                    [-105.8, -99.80, -38.10, -8.30] ])*fac[3]
    ...
    Col_Widths=np.array([0.70, 0.90, 0.70, 0.50, 0.60, 0.60, 0.60, 0.80, 0.50, 0.50, 0.50, 0.40, 0.50, 0.40])
    Col_Depths=np.array([0.50, 0.40, 0.30, 0.50, 0.50, 0.30, 0.40, 0.30, 0.30, 0.40, 0.25, 0.30, 0.25, 0.25])
    ...
    Col_Rebars=[ [6, fi16, 3, 2, fi16, 3, 2, fi16, 3, 2, fi16, 3, 6, fi16, 3],
                    [6, fi16, 3, 2, fi14, 3, 0, fi16, 3, 2, fi16, 3, 6, fi16, 3],
                    [5, fi16, 3, 0, fi14, 3, 2, fi16, 3, 0, fi14, 3, 5, fi16, 3],
                    [5, fi16, 3, 2, fi16, 3, 0, fi16, 3, 2, fi16, 3, 5, fi16, 3],
                    [6, fi16, 3, 2, fi16, 3, 0, fi16, 3, 2, fi16, 3, 6, fi16, 3],
                    [4, fi16, 3, 0, fi14, 3, 2, fi16, 3, 0, fi14, 3, 4, fi16, 3],
                    [4, fi16, 3, 2, fi14, 3, 0, fi16, 3, 2, fi16, 3, 4, fi16, 3],
                    [5, fi16, 3, 0, fi14, 3, 2, fi16, 3, 0, fi14, 3, 5, fi16, 3],
                    [3, fi16, 3, 0, fi14, 3, 2, fi16, 3, 0, fi14, 3, 3, fi16, 3],
                    [4, fi16, 3, 0, fi14, 3, 2, fi16, 3, 0, fi14, 3, 4, fi16, 3],
                    [3, fi16, 3, 0, fi14, 3, 2, fi16, 3, 0, fi14, 3, 3, fi16, 3],
                    [2, fi16, 3, 0, fi14, 3, 2, fi16, 3, 0, fi14, 3, 2, fi16, 3],
                    [3, fi16, 3, 0, fi14, 3, 2, fi16, 3, 0, fi14, 3, 3, fi16, 3],
                    [2, fi16, 3, 0, fi14, 3, 2, fi16, 3, 0, fi14, 3, 2, fi16, 3] ]

    # Slab thickness	es of the T-beam sections
    Hs=[0.12, 0.12, 0.12, 0.12]

    # Clear beam depth from the bottom face of the slab to the bottom face of the beam (m)
    Hb=[0.58, 0.58, 0.58, 0.58]

    # Effective slab width (m)
    Bs=[1.00, 1.00, 1.00, 1.00]

    # Beam width at the bottom (m)
    Bw=[0.25, 0.25, 0.25, 0.25]
    ...
    #       Number of Bottom Rebars    , Diameter, MatTag
    Beam_Rebars=[ [6, fi12, 3, 6, fi8, 3, 0, fi18, 3, 3, fi12, 3],
                    [6, fi12, 3, 6, fi8, 3, 0, fi12, 0, 4, fi12, 3],
                    [5, fi12, 3, 6, fi8, 3, 0, fi12, 0, 3, fi12, 3],
                    [5, fi12, 3, 6, fi8, 3, 0, fi12, 0, 3, fi12, 3]]
    ...
    acc_data=np.loadtxt('GMfile/'+filename)
    nPts =len(acc_data)
    ...
    #                    Tag
    os.timeSeries('Path', 2, '-filePath', 'GMfile/' + filename, '-dt', dt, '-factor', 0.01)
    ...
    # Saving the objects:
    with open('Out/' + filename[:-4] + '_Analysis' + str(pid) + str(record_no) + '.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump([floor_disps, base_shears, vertical_reactions, tCurrent, fac], f)
